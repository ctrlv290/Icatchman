...
$get_pconum = $_POST["pco_num"]; //시리얼번호
$sqlOption = " select * from maintain where site_idx = '".$get_pconum."'";

$g_cap_top = $_POST["cap_top"];
if($g_cap_top == 1){ $sqlOption = $sqlOption . " AND cap_top = '1'"; }

$g_cap_round = $_POST["cap_round"];
if($g_cap_round == 1){ $sqlOption = $sqlOption . " AND cap_round = '1'"; }

$g_cap_bottom = $_POST["cap_bottom"];
if($g_cap_bottom == 1){ $sqlOption = $sqlOption . " AND cap_bottom = '1'"; }

$g_motor_top = $_POST["motor_top"];
if($g_motor_top == 1){ $sqlOption = $sqlOption . " AND motor_top = '1'"; }

$g_motor_sinc = $_POST["motor_sinc"];
if($g_motor_sinc == 1){ $sqlOption = $sqlOption . " AND motor_sinc = '1'"; }

$g_motor_spider = $_POST["motor_spider"];
if($g_motor_spider == 1){ $sqlOption = $sqlOption . " AND motor_spider = '1'"; }

$g_round_pan = $_POST["round_pan"];
if($g_round_pan == 1){ $sqlOption = $sqlOption . " AND round_pan = '1'"; }

$g_motor_bottom = $_POST["motor_bottom"];
if($g_motor_bottom == 1){ $sqlOption = $sqlOption . " AND motor_bottom = '1'"; }

$g_plasticfan = $_POST["plasticfan"];
if($g_plasticfan == 1){ $sqlOption = $sqlOption . " AND plasticfan = '1'"; }

$g_reducer = $_POST["reducer"];
if($g_reducer == 1){ $sqlOption = $sqlOption . " AND reducer = '1'"; }

$g_cutting = $_POST["cutting"];
if($g_cutting == 1){ $sqlOption = $sqlOption . " AND cutting = '1'"; }

$g_breaker = $_POST["breaker"];
if($g_breaker == 1){ $sqlOption = $sqlOption . " AND breaker = '1'"; }

$g_lamp_normal = $_POST["lamp_normal"];
if($g_lamp_normal == 1){ $sqlOption = $sqlOption . " AND lamp_normal = '1'"; }

$g_lamp_led = $_POST["lamp_led"];
if($g_lamp_led == 1){ $sqlOption = $sqlOption . " AND lamp_led = '1'"; }

$g_lamp_uv = $_POST["lamp_uv"];
if($g_lamp_uv == 1){ $sqlOption = $sqlOption . " AND lamp_uv = '1'"; }

$g_syear = $_POST["syear"];
$g_smonth = $_POST["smonth"];
$g_sday = $_POST["sday"];
$g_eyear = $_POST["eyear"];
$g_emonth = $_POST["emonth"];
$g_eday = $_POST["eday"];

$fs_day = $g_syear . "-" . $g_smonth . "-" . $g_sday;
$es_day = $g_eyear . "-" . $g_emonth . "-" . $g_eday;

$sqlOption = $sqlOption . " AND n_day >= '".$fs_day."' AND n_day <='".$es_day."'";
$sqlOption = $sqlOption . " order by idx asc ";
...
/*
$nowtoday = date("Y-m-d");
$sqlMaintain = " select * from maintain where site_idx = '".$get_pconum."' order by idx asc ";
$rs_maintain 	= query( $sqlMaintain );
*/

//$sqlOption = "select * from maintain where site_idx = '5' AND n_day >= '2018-9-1' AND n_day <='2018-9-10' order by idx asc ";
$rs_maintain 	= query( $sqlOption );
$cnt = 0;
